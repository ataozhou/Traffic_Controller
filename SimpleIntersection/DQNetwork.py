...
class DQNetwork:
	...
	def __init__(self, state_shape, action_shape, learning_rate, name):
		...
		with tf.variable_scope(self.name):
			...
			self.value =  {'weights':tf.Variable(tf.random_normal([self.n_nodes_l2, 1]) * np.sqrt(2 / self.n_nodes_l2)), 'biases':tf.Variable(tf.random_normal([1]))}
			self.advantage = {'weights':tf.Variable(tf.random_normal([self.n_nodes_l2, self.action_shape]) * np.sqrt(2 / self.n_nodes_l2)), 'biases':tf.Variable(tf.random_normal([self.action_shape]))}
			...
			self.value = tf.add(tf.matmul(self.l2, self.value['weights']), self.value['biases'])
			self.advantage = tf.add(tf.matmul(self.l2, self.advantage['weights']), self.advantage['biases'])
